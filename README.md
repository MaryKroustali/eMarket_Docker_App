...
Για την υλοποιήση έγινε η αγορά των τεσσέρων προϊόντων που προστέθηκαν στο καλάθι κατά το entrypoint <ins>Add product to cart</ins>. Αρχικά δόθηκε λάθος πλήθος ψηφίων για τον 
κωδικό της κάρτας και έπειτα έγινε επιτυχημένη αγορά.

<img src="screenshots/buy.png">

### Entrypoint: Get order history
Το παρακάτω entrypoint δίνει στον χρήστη την δυνατότητα να δει το ιστορικό παραγγελιών του.

Ο χρήστης καλείται να δώσει το email του, ώστε να ελεγθεί αν έχει ιστoρικό παραγγελιών ή εάν είναι κενό `if "orderHistory" in user:`. Αν ο χρήστης έχει ιστορικό του εκτυπώνεται 
αν όχι εμφανίζεται αντίστοιχο μήνυμα.

Για την υλοποίηση του entrypoint αρχικά εμφανίζεται το ιστορικό για την παραγγελία που εκτελέστηκε στο προηγούμενο entrypoint, ενώ ο ίδιος χρήστης πραγματοποιεί άλλη μία αγορά, 
η οποία προστίθεται στο ιστορικό του.

<img src="screenshots/history1.png">
<img src="screenshots/history2.png">

### Entrypoint: Delete account
Το entrypoint αυτό δίνει την δυνατότητα σε κάποιον χρήστη να διαγράψει τον λογαριασμό του στο super market. 

Ο χρήστης δίνει αρχικά το email του, ώστε να αναζητηθεί αν υπάρχει τέτοιο email στη βάση, και αφού βρεθεί πραγματοποιείται η διαγραφή του `users.delete_one(user)`. Αν ο χρήστης 
δεν βρεθεί εμφανίζεται κατάλληλο μήνυμα.

Για την υλοποίηση, αρχικά, διαγράφεται ένας χρήστης και στην συνέχεια αναζητείται ξανα ο ίδιος χρήστης στην βάση.
...
Ζητείται το στοιχείο προς αλλαγή και το id του προϊόντος ώστε να ελεγχθεί η υπαρξή του στη βάση. Αν το προϊόν βρεθεί, ανάλογα το στοιχείο που πρέπει να τροποποιηθεί, το όνομα, 
την τιμή, την περιγραφή ή το απόθεμα, εκτελέιται το αντίστοιχο update query:
```python
 if "name" in data:
